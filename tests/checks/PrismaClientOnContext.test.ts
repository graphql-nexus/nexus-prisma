...
import { clearKleurFormat } from '../../src/lib/messenger/Messenger'
...
describe('instanceOf_duckType_fallback strategy:', () => {
  ...
  testIntegration({
    ...
    expect(result) {
      if (result.logs[0]) {
        result.logs[0] = result.logs[0]!.replace(/(.*imported from).*(is not the.*)/, '$1 <dynamic_path> $2').replace(clearKleurFormat, '')
      }
